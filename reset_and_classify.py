...
def validate_code(code: str, valid_codes: set) -> str:
    ...
    # For 4-part codes, check if valid or try to find match
    if code in valid_codes:
        return code

    # Try to find nearest valid code for 4-part codes
    parts = code.split('.')
    if len(parts) == 4:
        # First try the 3-part version
...
def classify_with_ollama(prompt: str, valid_codes: set, timeout: int = 180) -> tuple:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return parse_classification_response(text, valid_codes)
...
def parse_classification_response(text: str, valid_codes: set) -> tuple:
